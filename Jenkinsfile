    ...
    stages {
        ...
        stage('Trivy Security Scan') {
            steps {
                script {
                    if (env.BUILD_AUTH == 'true') {
                        sh "trivy image --skip-version-check --scanners vuln --exit-code 1 --severity CRITICAL ${env.DOCKER_HUB_USER}/chat-auth:${env.BUILD_NUMBER}"
                    }
                    if (env.BUILD_PROFILES == 'true') {
                        sh "trivy image --skip-version-check --scanners vuln --exit-code 1 --severity CRITICAL ${env.DOCKER_HUB_USER}/chat-profiles:${env.BUILD_NUMBER}"
                    }
                    if (env.BUILD_MESSAGING == 'true') {
                        sh "trivy image --skip-version-check --scanners vuln --exit-code 1 --severity CRITICAL ${env.DOCKER_HUB_USER}/chat-messaging:${env.BUILD_NUMBER}"
                    }
                    if (env.BUILD_FRONT == 'true') {
                        sh "trivy image --skip-version-check --scanners vuln --exit-code 1 --severity CRITICAL ${env.DOCKER_HUB_USER}/chat-front:${env.BUILD_NUMBER}"
                    }
                }
            }
        }
